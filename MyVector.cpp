...
private:
	...
	const string pass_end_warn_string = "You have passed the boundary of this vector, please check you code , please make sure the safety of this step.";//越界的异常语句，作为错误抛出使用
...
public:
	...
	void sort(bool is_increase = true) {
		...
		for (int i = length - 1; i >= 0; --i) {
			bool flag = true;
			for (int j = 0; j < i; ++j) {
				if (comparePattern(is_increase, operator[](j + 1), operator[](j))) {
					swap(j, j + 1);
					flag = false;
				}
			}
